...
void Image::loadFeatures(Mat &into) const
{
    ...
    for (int y = 0; y < image.size().height - PATCH_SIZE + 1; y += PATCH_STRIDE)
    {
        for (int x = 0; x < image.size().width - PATCH_SIZE + 1; x += PATCH_STRIDE)
        {
            ...
            extractFeatures(into, patch);
        }
    }
}

void Image::extractFeatures(cv::Mat &into, const cv::Mat &patch) const
{
    cv::Mat p = cv::Mat::zeros(1, FEATURE_VECTOR_LENGTH, CV_32F);
...
bool Image::loadVisualWords(cv::Mat &into, const cv::Mat &visualWords) const
{
    ...
    {
        int word = classifyWord(patches.row(patch), visualWords);
...
int Image::classifyWord(const cv::Mat &features, const cv::Mat &visualWords) const
{
    double min_dist = cv::norm(features, visualWords.row(0));
    int    min_ix   = 0;

    for (int word = 1; word < visualWords.size().height; word++)
    {
        double dist = cv::norm(features, visualWords.row(word));
        if (dist < min_dist)
        {
            min_dist = dist;
            min_ix   = word;
